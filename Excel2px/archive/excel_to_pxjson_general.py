...
def write_pxmetadata(schema: DetectedSchema, out_path: Path) -> Dict:
        ...
        measurements.append(
            {
                ...
                "code": code,  # PxBuild data-mapper uses this (key fix)
                "label": {"no": m, "en": m},
                "columnName": m,  # REQUIRED by PxBuild (matches parquet column)
                "showDecimals": 1,
    ...
    # --- coded dimensions ---
    # For each coded dim "base" we expect two parquet columns:
    #   base           (text/label column)
    #   base_kode      (code column)
    coded_dimensions = []
    for base in schema.coded_dims:
        coded_dimensions.append(
            {
                ...
                "label": {"no": base, "en": base},
                # PxBuild / PxMetadata model expects these:
                "codelistId": base,  # REQUIRED
                "pxcodesId": base,  # used by our setup; keep
                # IMPORTANT: both must exist in parquet
    ...
    # --- non-coded dims ---
    # Include all dims except:
    # - time column
    # - any *_kode helper columns
    # - the coded dimension text columns themselves (they are already in codedDimensions)
    other_dims = []
    coded_dim_set = set(schema.coded_dims)
    ...
    payload = {
        "dataset": {
            ...
            # IMPORTANT:
            # Must be WITHOUT ".parquet" if your config uses ".../{id}.parquet"
            "dataFile": schema.tableid,
            "timeDimension": {
                "dimensionId": schema.time_col,  # e.g. "aar"
                "columnName": schema.time_col,  # REQUIRED (prevents None)
