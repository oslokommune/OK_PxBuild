"""
Convert Excel file  into a PX-compatible parquet + pxjson.
...
TIME COLUMN:
- Excel file must have a time column (e.g. År/år/aargang/year)
- Accepted header spellings (case/spacing doesn’t matter much): År, år, ar, aar, aargang, year.

CODED LABEL PAIRS:
- Coded label pairs (e.g. region_kode + region_navn) should be in adjacent columns, with the code column first, with identical headers.
    (Pandas will read duplicates as "region", "region.1", etc., and the script will rename to "region_kode", "region_navn".)

TITLE ROW:
- If the file has a title row above headers, the script tries to find the header row by scanning for a cell that normalizes to aar.
...
def is_measure_like_name(name: str) -> bool:
    """Treat columns as measures if their names strongly suggest values."""
    n = name.lower()
    return n in {
        "value",
        "values",
...
def looks_like_code_dimension(colname: str, series: pd.Series) -> bool:
    """
    Treat numeric columns as categorical codes if they look like IDs/codes.
...
def read_excel_smart(xlsx: Path) -> pd.DataFrame:
    ...
    1) reads a small preview with header=None
    2) finds the first row containing a time header (år/aar/year)
    3) re-reads using that row as header
    """
    ...
    for i in range(len(preview)):
        ...
        if any(v in {"ar", "aar", "aargang", "year"} for v in vals):
            header_row = i
            break

    if header_row is None:
        # fall back to standard read; caller will validate later
        return pd.read_excel(xlsx)

    df = pd.read_excel(xlsx, header=header_row)

    # Clean up obvious unnamed columns (common for the value column)
    new_cols = []
